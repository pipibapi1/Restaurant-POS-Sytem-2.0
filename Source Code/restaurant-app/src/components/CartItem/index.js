...
function CartItem(props){
    ...
    return (
        <div className={styles.cartItem} id={props.id}>
            <div>{props.nameItem}</div>
            <div className={styles.content}>
                <div className={styles.changeQuantity}>
                    <button className={styles.subBtn} onClick={sub1Item}>-</button>
                    <span className={styles.quantity}>
                        {curQuantity}
                    </span>
                    <button className={styles.addBtn} onClick={add1Item}>+</button>
                </div>
                <div className={styles.price}>{totalCartItemCost} VND</div>
            </div>
        </div>
        )
}
